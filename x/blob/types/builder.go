...
func (k *KeyringSigner) BuildSignedTx(builder sdkclient.TxBuilder, msg ...sdktypes.Msg) (authsigning.Tx, error) {
	...
	bytesToSign, err := authsigning.GetSignBytesAdapter(ctx, k.encCfg.TxConfig.SignModeHandler(), signing.SignMode_SIGN_MODE_DIRECT, signerData, builder.GetTx())
	...
	sigBytes, _, err := k.SignByAddress(addr, bytesToSign, signing.SignMode_SIGN_MODE_DIRECT)
	if err != nil {
		return nil, err
	}
