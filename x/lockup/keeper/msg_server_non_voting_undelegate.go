...
func (k msgServer) NonVotingUndelegate(ctx context.Context, msg *types.MsgNonVotingUndelegate) (*types.MsgNonVotingUndelegateResponse, error) {
	...
	res, err := k.ShareclassMsgServer.NonVotingUndelegate(ctx, &shareclasstypes.MsgNonVotingUndelegate{
	...
	for i, entry := range entries {
		if entry.CreationHeight == sdkCtx.BlockHeight() && entry.EndTime == res.CompletionTime.Unix() {
			entry.Amount = entry.Amount.Add(res.Amount.Amount)
	...
	if isNewEntry {
		entries = append(entries, &types.UnbondingEntry{
			...
			EndTime:          res.CompletionTime.Unix(),
	...
	found, coin := res.Rewards.Find(feeDenom)
	if found {
