package types

// DONTCOVER

import (
	"cosmossdk.io/errors/v2"
)

// x/lockup module sentinel errors
var (
	ErrInvalidSigner              = errors.Register(ModuleName, 1100, "expected gov account as only signer for proposal message")
	ErrInvalidTimeRange           = errors.Register(ModuleName, 1101, "invalid time range")
	ErrLockupAccountAlreadyExists = errors.Register(ModuleName, 1102, "lockup account already exists")
	ErrInsufficientUnlockedFunds  = errors.Register(ModuleName, 1103, "insufficient unlocked funds")
	ErrInvalidDuration            = errors.Register(ModuleName, 7, "invalid duration")
	ErrInvalidTimestamp           = errors.Register(ModuleName, 8, "invalid timestamp")
	ErrAlreadyListed              = errors.Register(ModuleName, 9, "lockup account already listed for sale")
	ErrNotListed                  = errors.Register(ModuleName, 10, "lockup account not listed for sale")
	ErrInvalidSale                = errors.Register(ModuleName, 11, "invalid sale: buyer and seller cannot be the same")
)
