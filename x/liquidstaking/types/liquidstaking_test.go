...
func TestBTokenToNativeTokenWithFee(t *testing.T) {
	testCases := []struct {
		bTokenAmount            sdkmath.Int
		bTokenTotalSupplyAmount sdkmath.Int
		netAmount               sdkmath.LegacyDec
		feeRate                 sdkmath.LegacyDec
		expectedOutput          sdkmath.LegacyDec
	}{
		// reward added case
		{
			bTokenAmount:            sdkmath.NewInt(100000000),
			bTokenTotalSupplyAmount: sdkmath.NewInt(5000000000),
			netAmount:               sdkmath.LegacyNewDec(5100000000),
			feeRate:                 sdkmath.LegacyMustNewDecFromStr("0.0"),
			expectedOutput:          sdkmath.LegacyMustNewDecFromStr("102000000.0"),
		},
		// reward added case with fee
		{
			...
			bTokenTotalSupplyAmount: sdkmath.NewInt(5000000000),
			netAmount:               sdkmath.LegacyNewDec(5100000000),
			feeRate:                 sdkmath.LegacyMustNewDecFromStr("0.005"),
			expectedOutput:          sdkmath.LegacyMustNewDecFromStr("101490000.0"),
		},
		// slashed case
		{
			bTokenAmount:            sdkmath.NewInt(100000000),
			bTokenTotalSupplyAmount: sdkmath.NewInt(5000000000),
			netAmount:               sdkmath.LegacyNewDec(4000000000),
			feeRate:                 sdkmath.LegacyMustNewDecFromStr("0.0"),
			expectedOutput:          sdkmath.LegacyMustNewDecFromStr("80000000.0"),
		},
		// slashed case with fee
		{
			...
			bTokenTotalSupplyAmount: sdkmath.NewInt(5000000000),
			netAmount:               sdkmath.LegacyNewDec(4000000000),
			feeRate:                 sdkmath.LegacyMustNewDecFromStr("0.001"),
			expectedOutput:          sdkmath.LegacyMustNewDecFromStr("79920000.0"),
		},
	}

	for _, tc := range testCases {
		require.IsType(t, sdkmath.Int{}, tc.bTokenAmount)
		require.IsType(t, sdkmath.Int{}, tc.bTokenTotalSupplyAmount)
		require.IsType(t, sdkmath.LegacyDec{}, tc.netAmount)
		require.IsType(t, sdkmath.LegacyDec{}, tc.feeRate)
		require.IsType(t, sdkmath.LegacyDec{}, tc.expectedOutput)
...
func TestNativeToBTokenTo(t *testing.T) {
	testCases := []struct {
		...
		{
			nativeTokenAmount:       sdkmath.NewInt(100000000),
			bTokenTotalSupplyAmount: sdkmath.NewInt(5000000000),
			netAmount:               sdkmath.LegacyNewDec(5000000000),
			expectedOutput:          sdkmath.NewInt(100000000),
		},
		{
			nativeTokenAmount:       sdkmath.NewInt(100000000),
			bTokenTotalSupplyAmount: sdkmath.NewInt(5000000000),
			netAmount:               sdkmath.LegacyNewDec(4000000000),
			expectedOutput:          sdkmath.NewInt(125000000),
		},
		{
			nativeTokenAmount:       sdkmath.NewInt(100000000),
			bTokenTotalSupplyAmount: sdkmath.NewInt(5000000000),
			netAmount:               sdkmath.LegacyNewDec(55000000000),
			expectedOutput:          sdkmath.NewInt(9090909),
		},
	}

	for _, tc := range testCases {
		require.IsType(t, sdkmath.Int{}, tc.nativeTokenAmount)
		require.IsType(t, sdkmath.Int{}, tc.bTokenTotalSupplyAmount)
		require.IsType(t, sdkmath.LegacyDec{}, tc.netAmount)
		require.IsType(t, sdkmath.Int{}, tc.expectedOutput)
	...
	testCases := []struct {
		...
		// active case 1
		{
			validator: stakingtypes.Validator{
				...
				Tokens:          sdkmath.NewInt(100000000),
				DelegatorShares: sdkmath.LegacyNewDec(100000000),
			},
			whitelisted:    true,
			tombstoned:     false,
			expectedOutput: true,
...
func (s *KeeperTestSuite) SetupTest() {
	...
	s.addrs = chain.AddTestAddrs(s.app, s.ctx, 10, sdkmath.NewInt(1_000_000_000))
	s.delAddrs = chain.AddTestAddrs(s.app, s.ctx, 10, sdkmath.NewInt(1_000_000_000))
...
func (s *KeeperTestSuite) CreateValidators(powers []int64) ([]sdk.AccAddress, []sdk.ValAddress, []cryptotypes.PubKey) {
	...
	num := len(powers)
	addrs := chain.AddTestAddrsIncremental(s.app, s.ctx, num, sdkmath.NewInt(1000000000))
	...
	for i, power := range powers {
		...
		newShares, err := s.app.StakingKeeper.Delegate(s.ctx, addrs[i], sdkmath.NewInt(power), stakingtypes.Unbonded, val, true)
		s.Require().NoError(err)
...
func (s *KeeperTestSuite) TestLiquidStake() {
	...
	params := s.keeper.GetParams(s.ctx)
	params.MinLiquidStakingAmount = sdkmath.NewInt(50000)
	s.keeper.SetParams(s.ctx, params)
	s.keeper.UpdateLiquidValidatorSet(s.ctx)
	...
	s.Require().Equal(newShares, sdkmath.LegacyZeroDec())
	s.Require().Equal(bTokenMintAmt, sdkmath.ZeroInt())
	...
	s.Require().Equal(sdkmath.LegacyZeroDec(), res[0].DelShares)
	s.Require().Equal(sdkmath.ZeroInt(), res[0].LiquidTokens)
	...
	s.Require().Equal(sdkmath.LegacyZeroDec(), res[1].DelShares)
	s.Require().Equal(sdkmath.ZeroInt(), res[1].LiquidTokens)
	...
	s.Require().Equal(sdkmath.LegacyZeroDec(), res[2].DelShares)
	s.Require().Equal(sdkmath.ZeroInt(), res[2].LiquidTokens)
	...
	s.Require().True(found)
	s.Require().Equal(proxyAccDel1.Shares, sdkmath.LegacyNewDec(16668)) // 16666 + add crumb 2 to 1st active validator
	s.Require().Equal(proxyAccDel2.Shares, sdkmath.LegacyNewDec(16666))
	s.Require().Equal(proxyAccDel2.Shares, sdkmath.LegacyNewDec(16666))
	...
	s.Require().Equal(balanceBeforeUBD.Amount, sdkmath.NewInt(999950000))
	ubdBToken := sdk.NewCoin(liquidBondDenom, sdkmath.NewInt(10000))
	...
	s.Require().Equal(bTokenBalance, sdk.NewCoin(liquidBondDenom, sdkmath.NewInt(50000)))
	...
	s.Require().NoError(err)
	s.Require().EqualValues(unbondedAmt, sdkmath.ZeroInt())
	...
	s.Require().Equal(bTokenBalanceAfter, sdk.NewCoin(liquidBondDenom, sdkmath.NewInt(40000)))
	...
	s.Require().Equal(sdkmath.LegacyNewDec(13335), proxyAccDel1.Shares)
	s.Require().Equal(sdkmath.LegacyNewDec(13333), proxyAccDel2.Shares)
	s.Require().Equal(sdkmath.LegacyNewDec(13333), proxyAccDel3.Shares)

	res = s.keeper.GetAllLiquidValidatorStates(s.ctx)
	s.Require().Equal(params.WhitelistedValidators[0].ValidatorAddress, res[0].OperatorAddress)
	s.Require().Equal(params.WhitelistedValidators[0].TargetWeight, res[0].Weight)
	s.Require().Equal(types.ValidatorStatusActive, res[0].Status)
	s.Require().Equal(sdkmath.LegacyNewDec(13335), res[0].DelShares)

	s.Require().Equal(params.WhitelistedValidators[1].ValidatorAddress, res[1].OperatorAddress)
	s.Require().Equal(params.WhitelistedValidators[1].TargetWeight, res[1].Weight)
	s.Require().Equal(types.ValidatorStatusActive, res[1].Status)
	s.Require().Equal(sdkmath.LegacyNewDec(13333), res[1].DelShares)

	s.Require().Equal(params.WhitelistedValidators[2].ValidatorAddress, res[2].OperatorAddress)
	s.Require().Equal(params.WhitelistedValidators[2].TargetWeight, res[2].Weight)
	s.Require().Equal(types.ValidatorStatusActive, res[2].Status)
	s.Require().Equal(sdkmath.LegacyNewDec(13333), res[2].DelShares)
	...
	s.Require().EqualValues(alt, sdkmath.NewInt(40001))
}
