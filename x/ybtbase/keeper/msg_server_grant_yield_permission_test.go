...
func TestMsgServerGrantYieldPermission(t *testing.T) {
	tests := []struct {
		name     string
		setup    func(ctx sdk.Context, k keeper.Keeper)
		msg      *types.MsgGrantYieldPermission
		wantErr  bool
		errMsg   string
		validate func(t *testing.T, ctx sdk.Context, k keeper.Keeper)
	}{
		{
			name: "successful grant permission",
			setup: func(ctx sdk.Context, k keeper.Keeper) {
		...
		{
			name: "grant permission to already permitted user",
			setup: func(ctx sdk.Context, k keeper.Keeper) {
				// Create permissioned token
				token := types.Token{
					Creator:      testAddress,
			...
			wantErr: false, // Should succeed even if already granted
			validate: func(t *testing.T, ctx sdk.Context, k keeper.Keeper) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resp, err := ms.GrantYieldPermission(ctx, tt.msg)
			if tt.wantErr {
				require.Error(t, err)
				if tt.errMsg != "" {
