...
func (slashingModule) DefaultGenesis(cdc codec.JSONCodec) json.RawMessage {
	...
	params.MinSignedPerWindow = sdkmath.LegacyNewDecWithPrec(75, 2) // 75%
	params.SignedBlocksWindow = 5000
	params.DowntimeJailDuration = time.Minute * 1
	params.SlashFractionDoubleSign = sdkmath.LegacyNewDecWithPrec(2, 2) // 2%
	params.SlashFractionDowntime = sdkmath.LegacyZeroDec()              // 0%
...
func (distributionModule) DefaultGenesis(cdc codec.JSONCodec) json.RawMessage {
	...
	params.BaseProposerReward = sdkmath.LegacyZeroDec()  // 0%
	params.BonusProposerReward = sdkmath.LegacyZeroDec() // 0%
